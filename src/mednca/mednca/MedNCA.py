...
from src.mednca.mednca.BackboneNCA import BackboneNCA
import random
...
class MedNCA(nn.Module):
    ...
    """
    def __init__(self, device, channel_n: int = 32, fire_rate: float = 0.5, steps: int = 64, hidden_size: int = 128, input_channels: int = 1, output_channels: int = 1, batch_duplication: int = 1):
        r"""Init function
            #Args:
    ...
    def resize4d(self, img: torch.Tensor, size: tuple = (64, 64), factor: int = 4, label: bool = False) -> torch.Tensor:
        ...
        if img.dim() == 4:
            ...
            # Compute the scale factors for height and width
            ...
            # Reshape to 5D tensor (batch, channels, depth, height, width)
            img = img.permute(0, 3, 1, 2)  # Permute to [batch, channels, height, width]

            # Resize the image
            if label:
                img = F.interpolate(img, size=(resized_height, resized_width), mode='nearest')
            else:
            ...
            img = F.pad(img, (pad_width, pad_width, pad_height, pad_height))

            # In case the padding is odd, crop the extra pixels
            img = img[:, :, :new_height, :new_width]

            # Permute back to [batch, height, width, channels]
            img = img.permute(0, 2, 3, 1)
            
            # Ensure the channels dimension matches the expected 32 channels
            if img.size(3) != 32:
                img = img[:, :, :, :32]  # Truncate or pad the channels to match 32

        else:
            raise ValueError("Input tensor is not 4D")
